##Write a program in Python, to read the Tweet Dataset from MongoDB. Then for each 
#tweet, extract words from its ‘text’ field and obtain the category of words. Next, for each 
#Tweet, store the pairs of extracted word and the category of the word (e.g., a pair is 
#“train,NN”) to the existing “keyword” field in a Tweet in a comma-separated value (CSV) 
...
def find_keywords_in_text(text: str) -> list:  
    ...
    return keywords  
def keywords_to_csv(keywords: list) -> str:  
...
for t in tweets.find():  
    ...
    csv: str = keywords_to_csv(keywords)  
    tweets.update({"user.id":t["user"]["id"]},{"$set":{"keyword":csv}})